...
treeNJ <- NJ(BV.fst)  ##NJ tree of pairwise population Fst



##bootstrap suppport of all branches
##popnames
plot(treeNJ, "unrooted", main="NJ")  ##
```

Create consensus sequences for each population. 

Convert vcf to phylip using pgdspider. Then perform mulitple alignments of sequences online (MAFFT). 

Copy the multiple alignments in fasta formate to flashdisk. Align using BioEdit on red HP computer. Generate consensus sequences for each population. Copy onto flashdisk and back to mac. 

```
Bombina <- read.phyDat("BV71.oneindiv.perpop.fasta", format="fasta", type="DNA")   #concensus fasta sequences for each population
