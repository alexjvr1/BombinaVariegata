...
I'm using information from http://popgen.nescent.org/PopDiffSequenceData.html#pairwise-fst to test the significance of fst. 

First I generate 100 permutations of the genind object. Then I test whether the differentiation statistic is significantly different

```
library(mmod)
library(hierfstat)

bootstrap <- chao_bootstrap(BV.71.new, nreps=100)
summary(bootstrap)
    Length Class  Mode
BS  100    -none- list
obs   1    genind S4  

summarise_bootstrap(bootstrap, pairwise.fst)
```
...
myGtest <- global.rtest(BV.natural.scaled$tab,mySpca.BVscaled$lw,nperm=999)  ##test for global structure
myGtest
...
5.073201e+00 2.273581e-02 8.558441e-07 


myLtest <- local.rtest(BV.natural.scaled$tab,mySpca.BVscaled$lw,nperm=999)  ##test for local structure
myLtest
...
Based on 999 replicates
Simulated p-value: 0.739 
Alternative hypothesis: greater 

      Std.Obs   Expectation      Variance 
-6.985747e-01  2.314292e-02  9.168720e-07 
...
Test for a barrier to geneflow using PopGenReport

/Users/alexjvr/2016RADAnalysis/Bombina/BV234/Analyses_20161128/PopGenReport
...
coords <- read.table("BV38.coords", header=T)
coords

gendata <- read.structure("BV38.monmonier.str")
gendata
...
require(dismo)
longlat <- as.matrix(coords[,1:2])
longlat
xy <- Mercator(longlat)
gendata@other$xy <- NULL
gendata@other$xy <- xy

gendata@pop
pop <- read.table("BV38.pop", header=T)
pop.factor <- as.factor(pop$pop)
pop.factor
gendata@pop <- pop.factor


e <- extent(range(longlat[,1], range(longlat[,2]))
)
e <- extent(range(longlat[,1]), range(longlat[,2]))
e

tasmap <- gmap(e, style="feature:all|element:labels|visibility:off", maptype="roadmap")
plot(tasmap)
points(gendata@other$xy)

results3 <- landgenreport(gendata, tasmap, "Gst.Hedrick", NN=4, mk.pdf=F)
numpaths <- length(results3$leastcost$paths[[1]])

for (i in 1:numpaths)
{
lines(results3$leastcost$paths[[1]][[i]])
}
dummy<-lapply(results3$leastcost$paths[[1]], function (x) lines(x))


results3$leastcost$mantel.tab
